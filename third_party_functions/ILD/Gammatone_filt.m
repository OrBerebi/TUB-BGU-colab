...
function [x_pos_grad,f_c,n] = Gammatone_filt(x_l, x_r, f_lim, fs)
...
for k = 1:n
    ...
    f_lim_id
    % get ERB error
    X_out_L(k,:) = 10*log10(X_L);
    X_out_R(k,:) = 10*log10(X_R);

end
x_out(:,:,1) = X_out_L;
x_out(:,:,2) = X_out_R;

x_pos_grad = zeros(n-1,c,2);
for k = 1:n-1
    x_pos_grad(k, :, :) = x_out(k + 1, :, :) - x_out(k, :, :);
